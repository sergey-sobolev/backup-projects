...
2. Отредактируйте `backup-config.yaml`:
   ...
   - `sources` — список каталогов: строка или объект с `path`, опционально `name` и **`mode`** (режим по умолчанию для целей этого источника). Цели: **`target`** (одна строка) или **`targets`** (список). Элемент `targets` — либо строка пути (режим = `mode` источника или `default_mode`), либо объект **`{ target: путь, mode?: update|copy|tgz }`**; у объекта без `mode` используется `mode` источника. Если заданы и `target`, и `targets`, учитывается только **`targets`**
